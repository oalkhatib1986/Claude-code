...
- **THE LIBRARY IS THE PROGRAMME.** `const PROG` embeds every session from
  ATHLETE_PROGRAMMING.xlsx verbatim: `[block, week, day, stype, dateISO, weekLabel,
  [[partName,text]...]]`, 334 sessions, regenerated by `extract_prog.py` (scratchpad)
  which verifies every cell word-for-word against the sheet before emitting. Splice
  with the extractor, never by hand. `PROGV` is the programme's seedV: bump it when
  re-importing a corrected sheet and every UNEDITED loaded copy follows
  (`refreshSeeded` resolves through `libFind`, programme first, saved boards second).
  `progCfg(r)` builds a cfg on the fly — the programme is NOT copied into
  `af_presets_v1`; only boards the trainer saves live there. One-shot
  `loadProgrammeOnce` (flag `af_prog_v1`, own key, same lesson as `af_mw_v1`) deletes
  every seedV-stamped preset and loads today's session. `progTodayIx()`: exact date,
  else nearest dated session before today. Suites written against the boot DEFAULTS
  pin BOTH `af_mw_v1` and `af_prog_v1` before their first reload. `prog.js` gates the
  data byte-for-byte; `progui.js` the picker/editor/Today flows; `reseed.js` now
  gates the stamp contract against programme labels. `sis.js`/`mw.js` retired with
  their boards.
- **AN AS-WRITTEN PART IS THE COACH'S OWN WORDS.** The programme is prose — RPE
  schemes, supersets, clusters — that no builder should mangle: items carry
  `text` (+`dur`, 0 = untimed) instead of exercises, `exLines` renders every line
  verbatim, `partLabel` uses the first line, and Setup edits it in one textarea
  (`+ As written`). A board whose blocks are all untimed is a WHITEBOARD: no clock,
  no rotation, start button says so, and the summary chips drop rounds/format/total
  ("N parts"). Empty dangling "PART C" headers in the sheet are dropped at
  extraction (22 of them), counted and sanctioned in the verifier — a header with
  nothing under it is noise, not content.
